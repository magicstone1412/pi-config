...
Use `read`, `bash`, and `rg` for quick facts, and use Superconductor scouting or parallel interface design whenever it materially helps the review. Follow the Superconductor skill and live instructions, give every Pi role the exact run handshake and deterministic artifact path, then wait for, read, and verify every actual role result before fan-in.
