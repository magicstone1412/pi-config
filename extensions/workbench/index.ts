...
export default function workbenchExtension(pi: ExtensionAPI): void {
  ...
  pi.registerTool({
    ...
    promptGuidelines: [
      "Use launch_review_agent for standard /plan and direct code reviews with verified Claude Code, model claude-fable-5, and reasoning high; use another provider only when the human explicitly requests it.",
    ...
    async execute(_id, params, signal, _update, ctx) {
      ...
      const reviewContract = `\n\n## Durable SC review handoff (required)\nYou are the read-only reviewer for Workbench run ${JSON.stringify(active.runId)} at run root ${JSON.stringify(active.root)} with label ${JSON.stringify(params.label)}. Do not edit implementation files, commit, push, or launch other agents.\n\nRead the run's plan.md, todo records, and relevant worker/fix artifacts directly from that run root before assessing the code and commits. Run \`sc instructions review\` first. Inspect \`sc worktree review-checklist --json\`, \`sc worktree diff-summary --json\`, and every open review thread via review-list plus review-get. Review the actual diff and run targeted verification. Publish every actionable finding with \`sc worktree review-add\` at the relevant file/line. Every finding body must contain ${JSON.stringify(reviewerTag)}, a P0-P3 priority, the Workbench run ID, impact, and a concrete fix. Do not leave the findings only in terminal output.\n\nAfter all findings are published and verified with review-list/review-get, publish exactly one final file-anchored review comment. If approved, its body must begin \`[APPROVED] ${marker} ${reviewerTag}\`. If changes are needed, its body must begin \`${marker} ${reviewerTag} [NEEDS CHANGES]\`. In either case, include the run ID, initial open comment IDs and classifications, replies/resolutions, published finding IDs, remaining actionable IDs, and verification commands/results. Verify the final comment with review-get. The unique marker ${JSON.stringify(marker)} is the only completion signal Workbench accepts; terminal idle or a prose answer is not completion. End with a concise terminal summary only after the comments are verified.`;
      const launched = await launchAgent(
        scExec,
