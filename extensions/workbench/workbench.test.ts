...
describe("repository and run storage", () => {
  test("uses one repository identity from nested paths", async () => {
    ...
    const fromNestedPath = await resolveRepository(
      path.join(projectRoot, "extensions", "workbench"),
    );
...
describe("automatic Superconductor workspace membership", () => {
  test("detects managed workspace context and ignores ordinary Pi sessions", () => {
    ...
    ).toBe(path.basename(projectRoot));
  });
...
describe("artifacts", () => {
  ...
  test("rejects artifact reads and writes through symbolic links", async () => {
    ...
    await expect(writeArtifact(root, "artifacts/linked/escaped.md", "escape")).rejects.toThrow(
      "symbolic link",
    );
    await expect(readArtifact(root, "artifacts/linked/secret.md")).rejects.toThrow("symbolic link");
  });
});

describe("delegated agent reports", () => {
  test("writes, replaces, reads, and clears durable parent reports", async () => {
    ...
    expect(
      (await listArtifacts(root)).some((artifact) => artifact.path.includes("agent-reports")),
    ).toBe(false);
...
describe("todos", () => {
  ...
  test("allows only an audited coordinator force release", async () => {
    ...
    await expect(claimTodo(root, todo.id, worker, "session-a")).rejects.toThrow("cannot reclaim");
  ...
  test("reserves one worker launch per todo and requires unique recovery labels", async () => {
    ...
    await claimTodo(root, todo.id, worker, "session-a");
    await forceReleaseTodo(root, todo.id, coordinator, "coordinator-session", "worker disappeared");
    await expect(
...
describe("SC command adapter", () => {
  ...
  test("builds safe Pi and external-provider launch argv with multiline prompts", () => {
    ...
    const externalArgs = buildLaunchAgentArgs(
      {
        label: "run-reviewer",
        provider: "claude",
        prompt,
    ...
    expect(externalArgs[externalArgs.indexOf("--provider") + 1]).toBe("claude");
    expect(externalArgs[externalArgs.indexOf("--model") + 1]).toBe("claude-fable-5");
  });

  test("keeps transient idle and premature reports nonterminal until durable state agrees", () => {
    expect(classifyDurableHandoff("in_progress", undefined)).toBe("waiting");
    expect(classifyDurableHandoff("in_progress", "done")).toBe("waiting");
    expect(classifyDurableHandoff("done", undefined)).toBe("completed");
    expect(classifyDurableHandoff(undefined, "done")).toBe("completed");
    expect(classifyDurableHandoff("blocked", undefined)).toBe("needs_input");
    expect(classifyDurableHandoff("in_progress", "needs_input")).toBe("needs_input");
  });

  test("notifies once for missing completion and never for interactive idle", () => {
    const now = 200_000;
    expect(
      shouldNotifyMissingCompletion(
        { interactive: false, idleNoticeSent: false, idleSince: 80_000 },
        now,
      ),
    ).toBe(true);
    expect(
      shouldNotifyMissingCompletion(
        { interactive: false, idleNoticeSent: true, idleSince: 10_000 },
        now,
      ),
    ).toBe(false);
    expect(
      shouldNotifyMissingCompletion(
        { interactive: true, idleNoticeSent: false, idleSince: 10_000 },
        now,
      ),
    ).toBe(false);
    expect(
      shouldNotifyMissingCompletion(
        { interactive: false, idleNoticeSent: false, idleSince: 80_001 },
        now,
      ),
    ).toBe(false);
  });

  test("requires an authored exact-prefix SC review completion marker", async () => {
    const token = "review-token";
    const marker = reviewCompletionMarker(token);
    const reviewerTag = "[REVIEWER:run-reviewer]";
    const comments = extractReviewComments({
      response: {
        comments: [
          {
            id: "finding-1",
            body: `${reviewerTag} [P1] Finding quotes ${marker}`,
            author: "claude",
            file: "src/a.ts",
            start_line: 4,
            end_line: 4,
          ...
          {
            id: "wrong-author",
            body: `[APPROVED] ${marker} ${reviewerTag}`,
            author: "human",
            file: "src/a.ts",
            status: "Open",
          },
    ...
    expect(buildListReviewCommentsArgs()).toEqual(["worktree", "review-list", "--json"]);
    ...
    expect(findReviewCompletion(comments, token, reviewerTag, "claude")).toEqual({
      comment: expect.objectContaining({ id: "complete-1" }),
      verdict: "APPROVED",
    });
    expect(findReviewCompletion(comments, "stale-token", reviewerTag, "claude")).toBeUndefined();

    const calls: Array<{ cwd?: string }> = [];
    const exec: ScExecutor = async (_command, _args, options) => {
      calls.push({ cwd: options?.cwd });
      return {
        stdout: JSON.stringify({ response: { comments: [] } }),
        stderr: "",
        code: 0,
        killed: false,
      };
    };
    await listReviewComments(exec, "/tmp/review-worktree");
    expect(calls).toEqual([{ cwd: "/tmp/review-worktree" }]);
  });
  ...
  test("retries idle timeouts internally, then reads with one abort signal", async () => {
    ...
    const result = await waitForAgent(exec, input, projectRoot, controller.signal, (update) =>
      progress.push(update.attempts),
    );
  ...
  test("retries label registration and does not accept pre-start idle", async () => {
    ...
    const exec: ScExecutor = async (_command, args) => {
      ...
      if (args[0] === "agents") {
        ...
        if (getAttempt === 1) {
          return {
            stdout: JSON.stringify({
              response: {
                target_error: { message: "agent label 'worker' not found", code: "not_found" },
              },
            }),
            stderr: "",
            code: 3,
        ...
        return {
          stdout: JSON.stringify({
            response: { agent: { state, phase: state === "working" ? "running" : "idle" } },
          }),
          stderr: "",
          code: 0,
          killed: false,
  ...
  test("does not read after a failed wait", async () => {
    ...
    const exec: ScExecutor = async (_command, args) => {
      ...
      return {
        stdout: JSON.stringify({
          response: { targets: [{ target_error: { message: "provider failed" } }] },
        }),
        stderr: "",
        code: 3,
        killed: false,
    ...
    await expect(waitForAgent(exec, { target: "label:worker" }, projectRoot)).rejects.toThrow(
      "delegated agent failed or became unavailable: provider failed",
    );
  ...
  test("extracts launch identifiers and Pi session files from SC responses", () => {
    ...
    expect(extractAgentSessionFile({ session_id: "/tmp/direct-session.jsonl" })).toBe(
      "/tmp/direct-session.jsonl",
    );
...
describe("tool rendering", () => {
  ...
  test("summarizes todo records when collapsed and shows their full result when expanded", () => {
    ...
    expect(todoResultText(result, false)).toBe("TODO-001: Inspect binary file format (open)");
  ...
  test("formats compact coordinator and worker Workbench status", () => {
    ...
    expect(workbenchStatusText(coordinator)).toBe("WB native-sc-tools-for-wor… · coordinator");
    const worker = {
      ...coordinator,
      role: "worker",
    ...
    expect(workbenchStatusText(worker)).toBe("WB native-sc-tools-for-wor… · worker · TODO-002");
  ...
  test("renders a bounded live agent panel with compact Workbench identities", () => {
    ...
    const waiting = renderAgentPanelLines([waitingItem], 48, theme, 1_000_000);
    const waitingLater = renderAgentPanelLines([waitingItem], 48, theme, 1_120_000);
    expect(waiting[1]).toContain("00:15");
    expect(waitingLater[1]).toContain("00:15");
    expect(waiting[1]).toContain("waiting · 12 turns · $0.84");

    const coloredTheme = { fg: (color: string, text: string) => `<${color}>${text}</${color}>` };
    const coloredWaiting = renderAgentPanelLines([waitingItem], 200, coloredTheme, 1_000_000);
    expect(coloredWaiting[1]).toContain("<warning> 00:15  Worker · TODO-001 </warning>");
...
describe("file locks", () => {
  test("takes over stale locks and does not clean up a replacement owner", async () => {
    ...
    const remaining = JSON.parse(await readFile(path.join(lockPath, "replacement"), "utf8")) as {
      token: string;
    };
