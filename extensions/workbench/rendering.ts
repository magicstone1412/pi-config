...
export function launchAgentToolResult(launched: LaunchAgentResponse, limits: ScResultLimits) {
  return {
    content: [
      {
        type: "text" as const,
        text: boundedScResultText(
          launched,
          {
    ...
    content: [
      {
        ...
        text: boundedScResultText(
          ...
          {
            retrievalGuidance:
              "Call wait_for_agent again with a smaller last value to retrieve a bounded transcript. Full structured wait/read data remains in tool details.",
          },
          limits,
        ),
      },
...
function panelElapsedTime(item: AgentPanelItem, now: number): string {
  const elapsed =
    item.activeElapsedMs === undefined
      ? now - item.startedAt
      : item.activeElapsedMs + (item.activeSince === undefined ? 0 : now - item.activeSince);
...
function panelBorderLine(
  ...
  const padding = " ".repeat(Math.max(0, contentWidth - panelTextWidth(boundedLeft) - rightWidth));
  return (
    theme.fg("accent", "│") +
    theme.fg(leftColor, boundedLeft) +
    padding +
    theme.fg(rightColor, boundedRight) +
    theme.fg("accent", "│")
  );
}
...
function panelMetrics(item: AgentPanelItem): string | undefined {
  ...
  const cost =
    item.metrics.cost < 0.01 && item.metrics.cost > 0
...
export function renderAgentPanelLines(
  ...
  for (const item of items) {
    const left = ` ${panelElapsedTime(item, now)}  ${agentPanelName(item.target)} `;
    const status = panelStatus(item);
    const leftColor = item.status === "waiting" ? "warning" : "muted";
    lines.push(panelBorderLine(left, leftColor, ` ${status.text} `, status.color, width, theme));
  }
